...
#include <irbackend/BasicBlock.h>
#include <irbackend/Constants.h>
#include <irbackend/Function.h>
#include <irbackend/IRBuilder.h>
#include <irbackend/LLVMContext.h>
#include <irbackend/Module.h>
#include <irbackend/Type.h>
...
class assembly_builder : public c1_recognizer::syntax_tree::syntax_tree_visitor
{
  ...
  private:
    ...
    void SetWhileBlocks(YAL::BasicBlock* body, YAL::BasicBlock* next){
        if(BlockStack.front()->while_body == nullptr && BlockStack.front()->while_next == nullptr){
            BlockStack.front()->while_body = body;
            BlockStack.front()->while_next = next;
        }
    }
