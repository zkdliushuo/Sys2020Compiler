...
void PromoteMemToReg::RenamePass(BasicBlock *BB, BasicBlock *Pred, RenamePassData::ValVector &IncomingVals, std::vector<RenamePassData> &Worklist){
    ...
    for (BasicBlock::iterator II = BB->begin(); II != BB->end();) {
        ...
        if (LoadInst *LI = cast<LoadInst>(I)) {
            ...
            // std::cout << "Before RAUW: Load inst's user size is " << size0 << std::endl;
            // std::cout << "Before RAUW: PhiNode inst's user size is " << size1 << std::endl;
            ...
            // std::cout << "After RAUW: PhiNode inst's user size is " << size1 << std::endl;
