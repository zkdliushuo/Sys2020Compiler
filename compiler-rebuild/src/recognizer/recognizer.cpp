...
recognizer::recognizer(const std::string &input_string) : ast(nullptr) { input = new ANTLRInputStream(input_string); }

recognizer::recognizer(std::istream &input_stream) : ast(nullptr) { input = new ANTLRInputStream(input_stream); }

std::shared_ptr<syntax_tree::syntax_tree_node> recognizer::get_syntax_tree() { return ast; }

recognizer::~recognizer() { delete input; }

bool recognizer::execute(error_reporter &_err) {
  C1Lexer* lexer = new C1Lexer(input);
