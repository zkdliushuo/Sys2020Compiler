...
SemiMachineBlock globalVariableInitilize(Constant *CV, int var_space) {
  ...
  if (auto AllZero = cast<ConstantAggregateZero>(CV)) {
    ...
    auto single_init = new SemiMachineInstruction(word_value, SemiMachineInstruction::SemiMachineType::Variable);
  ...
  if (auto CA = cast<ConstantArray>(CV)) {
    ...
    for (unsigned i = 0, e = CA->getNumOperands(); i != e; ++i) {
      ...
      InitBlock +=
          globalVariableInitilize(cast<Constant>(CA->getNumOperand(i).get()), var_space - CA->getNumOperands());
    }
  }
  return InitBlock;
}
...
void AddBranchLabels(Instruction *inst, BasicBlock *true_bb, BasicBlock *fasle_bb,
                     int in_scope = Instruction::Opcode::Or) {}
...
SemiMachineBlock basicBlockTransform(BasicBlock *basic_block_item) {
  ...
  auto inst = new SemiMachineInstruction(basic_block_name, SemiMachineInstruction::SemiMachineType::BBLabel);
...
SemiMachineBlock functionTransform(Function *function_item) {
  ...
  std::string func_label = "\t.text\n\t.align	2\n	.global	" + func_name + "\n\t.arch armv8-a\n\t.type	" +
                           func_name + ", %function";
  // SemiMachineInstruction *inst =
  //     new SemiMachineInstruction(func_label, SemiMachineInstruction::SemiMachineType::FuncLabel, 12);

  auto inst = new SemiMachineInstruction(func_label, SemiMachineInstruction::SemiMachineType::FuncLabel);
  ...
  auto func_size_inst = new SemiMachineInstruction(func_size, SemiMachineInstruction::SemiMachineType::Variable);
  // function_semi_machine_block.addInstruction(new SemiMachineInstruction(func_size, 0, SemiMachineInstruction::Nop_));
  function_semi_machine_block.addInstruction(func_size_inst);
...
SemiMachineBlock moduleTransform(Module *module_item) {
  ...
  auto mod_div_decl =
      new SemiMachineInstruction(label_for_div_mod_abi, SemiMachineInstruction::SemiMachineType::Variable);
