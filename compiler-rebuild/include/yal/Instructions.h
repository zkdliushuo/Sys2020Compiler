...
public:
  ...
  bool hasSamePtr(GetElementPtrInst *that) {
    if (!that) return false;
    return that->getNumOperand(0).get() == getNumOperand(0).get();
  }
...
public:
  ...
  BasicBlock *getSuccessor(unsigned i) {
    return cast<BasicBlock>(getNumOperand(-1 - i).get());
  }
...
public:
  ...
  int getCmp() { return getSubclassDataFromValue(); }
...
public:
  ...
  bool hasSideEffect(Value *V) { 
    ...
    for (int i = 0; i < getNumOperands() - 1; i++) {
      auto op = getNumOperand(i).get();
      if (op == V) return true;
      if (auto gep = cast<GetElementPtrInst>(V)) {
        if (gep->getNumOperand(0).get() == V) return true;
      }
    }
    return false;
  }
...
public:
  bool isPHISpilled = false;
  // AllocaInst *SpilledAI = nullptr;
